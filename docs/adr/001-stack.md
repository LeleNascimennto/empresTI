...
- **Deploy** — "Monolito" é de código, não de processo; na Vercel cada rota vira uma function; o que é único é o codebase e o deploy, não o runtime.
...
Este ADR não decide a troca de provedores, a separação de front e backend, a adoção de fila e worker, a migração para um plano pago ou o comportamento fora do contexto acadêmico do projeto. Ele também não decide reescrever o stack para atender consumidor externo, processamento longo, tempo real, ou escala de produção com requisitos diferentes dos que estão aqui explicitados.
